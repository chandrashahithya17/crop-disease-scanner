...
def generate_lime_explanation(img_array, predicted_class_index):
    try:
        ...
        axes[1].axis('off')
        fig.suptitle('LIME: Why the AI detected this disease', fontsize=13, fontweight='bold', color='#1B4332')
        plt.tight_layout()
...
def generate_shap_explanation(img_array, predicted_class_index):
    try:
        ...
        if isinstance(shap_values, list):
            ...
            # new format: single array with shape (batch, h, w, c, classes)
            if shap_values.ndim == 5:
                shap_img = shap_values[0, :, :, :, predicted_class_index]
            else:
